...
const RequestEmail = ({ onSubmitSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // email of user to the backend to request password reset
      const response = await axios.post('https://api.example.com/request-reset', { email });

      // On success, move to next step in flow (call parent function)
      if (response.status === 200) {
        onSubmitSuccess(email); // Pass it to the parent component for flow
      }
    } catch (err) {
      // Handle error if backend API fails
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="text-left">  {/* Text aligned to the left */}
            ...
            <input
              ...
              required
              className="mt-1 w-full px-4 py-2 bg-white rounded-lg border border-gray-300 focus:outline-none 
                           focus:ring-2 focus:ring-custom-yellow-400 focus:border-transparent placeholder-gray-400"
